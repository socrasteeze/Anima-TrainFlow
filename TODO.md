...
## RESUME / LOG-PERSISTENCE / GALLERY MERGE — shipped 2026-07-10

- [x] **Resume training** — `create_training_toml` always passes `save_state`, `save_last_n_steps_state=1`, `save_state_on_train_end` (app.py ~349–352), so every run leaves one resumable accelerate state dir under `training/output/<proj>/`. `find_resumable_state` (app.py ~829) picks the most recently modified `{proj}-step*-state` / `{proj}-state` dir; `start_training` adds `--resume <dir>` when the new "🔁 Continue from last checkpoint" checkbox (default on) is checked and a state dir exists. sd-scripts restores optimizer state, RNG, and step count from the state's `train_state.json` automatically — no `--initial_step` needed.
- [x] **Log/ETA survive page refresh** — training state moved off the click-generator onto module-level globals (`training_log_lines`, `training_eta_text`, `training_sample_dir`, `training_active`) drained by a background thread (`_training_worker`, app.py ~763) that also tees raw output to `training/output/<proj>/train.log`. A `gr.Timer` (1s) plus `ui.load` both call `poll_training_status` (app.py ~956) to rehydrate the UI regardless of which browser request is asking — a refresh no longer severs the log/preview/ETA feed or leaves the subprocess's stdout pipe undrained.
- [x] **Preview gallery merge** — the separate "Live Previews" gallery and "Checkpoint A/B Gallery" accordion (same underlying `sample/` folder, just two renderers) collapsed into one `preview_gallery` with a "Latest / By Step" `gallery_sort` radio (app.py ~1319). `refresh_gallery_for_mode` (app.py ~1211) drives both the manual refresh button and the sort toggle; the auto-updating timer only touches the gallery in "Latest" mode so it doesn't fight a user who's browsing "By Step".
- [x] **Low-VRAM preset removed** — pure data change, `training/presets.json`. No code referenced it by name.
- [x] **Cache-validity skip (opt-in)** — new "⚡ Skip cache validity check" checkbox (default OFF, app.py ~1283) threads `skip_cache_check` into `create_training_toml`. Left off by default: it only skips *content* validation (existence check always still runs), so it's unsafe if a dataset's images/captions were edited in place without clearing old cache files — user must opt in knowingly. This does not touch the "slowdown between runs" root cause identified by investigation (each run cold-starts a fresh subprocess + reloads multi-GB model weights from disk — inherent to the one-process-per-run architecture, not a leak; no VRAM/zombie-process issue found).
